...
class CompanyRegistration(Document):
	def before_save(self):
		...
		if self.company_nameas_per_your_gst:
			if frappe.get_all('Company Registration', filters={'company_nameas_per_your_gst': self.company_nameas_per_your_gst}):
				frappe.throw('Company already exists.')

		# Check for duplicate GST
		if self.gst:
			if frappe.get_all('Company Registration', filters={'gst': self.gst}):
